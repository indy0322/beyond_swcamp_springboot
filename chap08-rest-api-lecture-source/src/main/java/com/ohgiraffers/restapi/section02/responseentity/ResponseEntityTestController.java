...
public class ResponseEntityTestController {
    ...
    /* 설명.
     *  ResponseEntity란?
     *   HttpRequest를 응답하기 위해 httpEntity를 상속받아 HttpStatus, HttpHeader,HttpBody를
     *   정의하여 사용하는 클래스이다.
     *   (HTTP는 단순한 데이터 전송이 아니라 상태 정보를 포함한 통신 프로토콜이기 때문에 올바른 상태 코드와 헤더를 제공해야 클라이언트가 적절히 반응할 수 있다.)
    * */
    ...
    /* 설명. 1. ResponseEntity의 매개변수가 있는 생성자를 통해 ResponeEntity를 반환하는 핸들러 메소드 */
    @GetMapping("/users")
    ...
    @PostMapping("/user")
    public ResponseEntity<ResponseMessage> registUser(@RequestBody UserDTO newMember) {
    ...
    public ResponseEntity<?> modifyUser(@RequestBody UserDTO modifyMember, @PathVariable int userNo) {
        UserDTO foundUser =
                users.stream()
                        .filter(user -> user.getNo() == userNo)
                        .collect(Collectors.toList()).get(0);
